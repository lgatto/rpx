...
- Remove the generated subdir in the ftp_url when creating the PXDataset
  object \<2023-09-26 Tue\> (see issue
  [\#25](https://github.com/lgatto/rpx/issues/25)).
...
- Delete entries in default cache before running the unit tests. This
  assures that the instances matche any updates in the class definition.
...
- New
  [`pxSubmissionDate()`](https://lgatto.github.io/rpx/reference/PXDataset2.md)
  and
  [`pxPublicationDate()`](https://lgatto.github.io/rpx/reference/PXDataset2.md)
  accessor functions.
...
- Don’t rely on a PRIDE project’s README.txt files anymore, as it has
  been discontinued (see issue
  [\#21](https://github.com/lgatto/rpx/issues/21)). The files in a
  project are now listed from the remote ftp directory.
...
- Don’t run caching example as it replies on PRIDE which is failing too
  often for the tests to pass on all systems.

### rpx 2.3.2

- Provide fix for PRIDE migration and annotation discrepancies (see
  issue [\#17](https://github.com/lgatto/rpx/issues/17)). This fix will
  however lead to re-downloading some cached files due to different
  URLs. The fix might only be temporary, based on if/how PRIDE will
  address their inconsistencies.

### rpx 2.3.1

- Use
  [`BiocFileCache::bfcrpath`](https://rdrr.io/pkg/BiocFileCache/man/BiocFileCache-class.html)
...
- Annotate PXD012095 as returning an error (see
  [\#12](https://github.com/lgatto/rpx/issues/12)) \<2021-10-04 Mon\>

### rpx 2.1.10

- Caching PRIDE sitemap with all PXD projects.
- New
  [`pxinstruments()`](https://lgatto.github.io/rpx/reference/PXDataset2.md),
  [`pxptms()`](https://lgatto.github.io/rpx/reference/PXDataset2.md) and
  [`pxprotocols()`](https://lgatto.github.io/rpx/reference/PXDataset2.md)
  accessors.
...
- Improve
  [`pxCachedProjects()`](https://lgatto.github.io/rpx/reference/cache.md)
  output.
...
- New
  [`pxCacheInfo()`](https://lgatto.github.io/rpx/reference/PXDataset2.md)
...
- PXDatasets are also cached upon creation and retrieved from cache next
...
- New feature: PXDataset objects now query and store data (ref, tax,
  files, url) when generated instead of fetching these on the fly every
  time. This new feature has been added to circumvent the issues with
  data access (see [\#5](https://github.com/lgatto/rpx/issues/5)).

### rpx 2.1.1

- `pxannouced()` paused (see
  [\#7](https://github.com/lgatto/rpx/issues/7)).
...
- Switching to
  [`tools::R_user_dir()`](https://rdrr.io/r/tools/userdir.html) from
...
- Don’t ask for cache location in non-interactive mode. This forces the
  usage of the default cache location (rather than usage of a Rtmp
...
- Temporary fix for issue [\#5](https://github.com/lgatto/rpx/issues/5)
  (wrong URL from PRIDE server) \<2019-10-02 Wed\>
...
- `pxget` gains a `destdir` argument to specify the destination to which
  files should be downloaded \<2017-01-27 Fri\>
...
- fixing pxfiles ending by n Windows \[2014-03-05 Wed\]
